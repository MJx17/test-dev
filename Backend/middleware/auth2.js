...
async function checkUserPermissions(user, route, method) {
  ...
  // Now we check if the user's role matches the required role for this resource
  if (!user.role.permissions.some(permission =>
    resource.permissions.some(resPermission => resPermission.name === permission.name)
  )) {
    return false;  // Deny access if the user doesn't have the necessary permission for the resource
  }

  // Check if the user's role is associated with the resource permissions
  const roleHasAccess = resource.roles.some(role => role._id.toString() === user.role._id.toString());
  
  if (!roleHasAccess) {
    return false;  // Deny access if the user's role doesn't match the expected role for the resource
  }

  return true;  // Grant access if both the role and permissions match
}
...
async function findResource(route, method) {
  ...
  return await Resource.findOne({ route, method })
    ...
    .populate('roles');  // Populate roles for the resource
}
