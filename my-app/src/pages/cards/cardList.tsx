...
import SlideForm from '../../pages/cards/cardsForm';
...
const NoticeCardDisplay = () => {
  ...
  const [selectedNoticeCardId, setSelectedNoticeCardId] = useState<string | null>(null);
...
const handleSortChange = (e: SelectChangeEvent<string>) => {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      // Select NoticeCards where deletedAt is null or undefined
      setSelectedIds(
        noticecards
          .filter((noticecard) => noticecard.deletedAt === null || noticecard.deletedAt === undefined) // Only select NoticeCards that are not deleted
          .map((noticecard) => noticecard._id) // Get the IDs of those NoticeCards
      );
    } else {
      // Deselect all selected NoticeCards
  ...
  const handleOpenEditDialog = (id: string) => {
    const noticecard = noticecards.find((noticecard) => noticecard._id === id);
    if (noticecard) {
      setSelectedNoticeCardId(id);
  ...
  const handleUpdateNoticeCard = async () => {
    ...
    try {
      ...
      await getNoticeCards(); // Refresh NoticeCards list
      alert('NoticeCard updated successfully!');
    } catch (error) {
      console.error('Error updating NoticeCard:', error);
      alert('Failed to update NoticeCard. Please try again.');
    }
  ...
  return (  
   ...
   <div className='noticecards-menu-buttons'>
    ...
    <div className="noticecards-table-container" style={{ width: '100%', overflowX: 'auto' }}>
        ...
        <TableCell sx={{ width: '150px', padding: '10px' }}>
          ...
          >
            <img
              ...
              alt={noticecard.title || 'NoticeCard Image'}
              width="150"
              height="150"
  ...
  <div style={{ display: 'flex', flexWrap: 'wrap' }}>
    {paginatedNoticeCards.map((NoticeCard) => (
      <Card
        ...
        style={{
          width: '100%', // Ensure NoticeCard takes full width of container
          margin: '8px', // Add margin between NoticeCards
        }}
      >
        <CardContent>
          <div
          ...
          <div style={{ marginBottom: '8px' }}>
            ...
            <Typography
              variant="body2"
              className="NoticeCard-description"
              style={{
                textAlign: 'justify',
                whiteSpace: 'normal',  // Allow text to wrap
                wordWrap: 'break-word', // Break long words to fit in the container
          ...
          <div style={{ marginBottom: '8px' }}>
            ...
            <img
              src={NoticeCard.imageUrl as string}
              alt={NoticeCard.title || 'NoticeCard Image'}
              className="slides-table-NoticeCard-img"
              style={{
                objectFit: 'cover',
                width: '100%',
                maxHeight: '200px',
      ...
      <Dialog open={openEditDialog} onClose={handleCloseEditDialog}>
        <DialogTitle>Edit NoticeCard</DialogTitle>
        <DialogContent>
          <TextField
            label="Title"
            value={title}
      ...
      {/* Soft Delete Dialog */}
          <Dialog open={openSoftDeleteDialog} onClose={handleCloseSoftDeleteDialog}>
            <DialogTitle>Delete NoticeCard</DialogTitle>
            <DialogContent>
              <TextField
          ...
          <Box display="flex" justifyContent="center" marginTop="20px">
            <Pagination
              page={page}
              count={filteredNoticeCards.length} // Total number of NoticeCards
...
export default NoticeCardDisplay;
