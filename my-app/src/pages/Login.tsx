...
const ModalComponent: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isAuthenticated) {
      ...
      const toastId = 'not-logged-in-toast';
      if (!toast.isActive(toastId)) {
        toast.error('You are not logged in. Please log in!', { toastId });
      }
  
      // Redirect to the login page
      setTimeout(() => {
        navigate('/login'); // Ensure the login route is correctly set in your React Router
      }, 1500); // Set a timeout to show the error toast before redirecting
    }
